...
All nine are registered unconditionally by `require("runtime-analysis").setup()`
— the four below plus telemetry's five `*All` aliases. None is independently
opt-in the way telemetry's own auto-instrumentation (`opts.telemetry`) is.
...
| `:RA cancel` | none | Discards the in-flight request's eventual result and shows `✗ cancelled`. A *logical* cancel, not a process kill — see [`commands.md`](commands.md) for why. Warns, does not error, when nothing is in flight. |
| `:RA history` | none | `vim.ui.select` picker over this project's recorded sends (method/url/status/timestamp only — see [`commands.md`](commands.md)), newest first; picking one reopens it via `open_request`. |
| `:RA history clear` | none | Clears this project's recorded history. No confirmation prompt. |
| `:RA env [name]` | environment name, optional, `<Tab>`-completed | With `name`, selects it as the active environment `{{var}}` placeholders resolve against; with none, `vim.ui.select` over every name the project's env files define. See [`commands.md`](commands.md) for the file format. |
...
| `:RA provenance <path>` | dotted path, e.g. `vim.notify` | Who wrapped this function right now — exact for this plugin's own telemetry wraps, best-effort (a `debug.getinfo` source location) for anyone else's. See [`commands.md`](commands.md). |
...
| `:RA startup profile [runs]` | none | Starts Neovim `runs` times (default 5) under `--startuptime`, sequentially, and reports the per-file cost as a median with its spread beside it. The one measurement here that runs `--startuptime` rather than working around it -- and the only one that sees a file sourced before any plugin could have armed anything. `gf` opens the file on the row. |
...
| `:RA inspect <module>` | a `package.loaded` key, `<Tab>`-completed live | Walks a live module table: functions, tables, metatables, what's shadowed through `__index`. See [`commands.md`](commands.md). |
| `:RA usage` | none | Report keymap/command press counts collected since `:RA usage start`. See [`commands.md`](commands.md). |
...
| `:RA loaded snapshot <prefix> [name]` | module prefix, optional snapshot name | Persists a named capture of every currently-loaded module under `<prefix>` (`runtime-analysis.loaded`). See [`commands.md`](commands.md). |
| `:RA loaded snapshots <prefix>` | module prefix | Lists saved loaded snapshots for `<prefix>`, newest first. See [`commands.md`](commands.md). |
...
Built via [`lib.nvim.bindings.usercmd.composer`](https://github.com/StefanBartl/lib.nvim/blob/main/lua/lib/nvim/bindings/usercmd/composer/README.md)
...
Full reasoning: [`commands.md`](commands.md).
...
dropped first. Full reasoning: [`commands.md`](commands.md)'s own section on it.
...
[`commands.md`](commands.md)'s own section on it.
...
[`commands.md`](commands.md) for the full reasoning. A match is a plain notify; a
...
see [`commands.md`](commands.md) for the full reasoning, the same trap `:RA env`'s
own section states.
...
See [`commands.md`](commands.md) for the full reasoning.
...
See [`commands.md`](commands.md) for the three design questions this resolves.
...
`runtime-analysis.telemetry` itself, the same wrap mechanism the cost-vs-use
report ([`FEATURE_LOG.md`](FEATURE_LOG.md) §7.2) already reads, pointed at
editor input instead of code.
See [`commands.md`](commands.md) for the honest limits (only mappings set after
...
collide. See [`commands.md`](commands.md) and
...
| `:RATelemetry status` | the fleet board: one aligned row per namespace this plugin knows about — live this session or only ever persisted — with state, mode, wrapped/call/session counts, size on disk and start date, closed by a summary. `<CR>` on a row opens that namespace's own full report; `r` re-reads, `?` lists the keys (also shown in the window's winbar) |
...
*action* is triggered directly by a command. Eleven real registrations exist
across seven groups, every one of them opt-in plumbing behind telemetry, usage
tracking or an active stall measurement rather than something a keymap could
ever collide with.

**Behind telemetry and usage tracking** — installed when the feature is
...
| `VimEnter` (`once = true`) | `telemetry/lazy.lua` | `runtime_analysis_telemetry_extra` | Wraps + starts each configured `opts.telemetry.extra` target once the UI is up. Skipped entirely (a `vim.schedule` instead) when `setup()` runs after `VimEnter` has already happened. |
...
**Behind an active stall measurement** — all five live in one group,
`RuntimeAnalysisStartup`, created by `:RA startup start`/`watch` and deleted
again by `:RA startup report`. They exist only while a measurement is running,
and they only ever stamp the timeline; none of them changes anything.

| Event | Module | Stamps |
| --- | --- | --- |
| `VimEnter` | `startup/init.lua` | `VimEnter` on the timeline |
| `User VeryLazy` | `startup/init.lua` | lazy.nvim's `VeryLazy` |
| `User LazyLoad` | `startup/init.lua` | each plugin load, with lazy's own load time **and** the reason it loaded |
| `LspAttach` | `startup/init.lua` | each LSP client attach, by client name |
| `LspProgress` | `startup/init.lua` | the `begin` of each LSP progress report, by title |

None of these exist unless the corresponding feature is actually in use: a
setup with `opts.telemetry` unset, `:RA usage start` never run and no stall
measurement in progress installs not one of them.
